...
internal partial class Aircraft : IAircraft, IDisposable
{
    ...
    private readonly ManualResetEvent _triggerLnavInstruction = new(true);
    private readonly AutoResetEvent _triggerVnavInstruction = new(true);
    private readonly AutoResetEvent _triggerSpeedInstruction = new(true);
    private ConcurrentQueue<(InstructionType Instruction, object Data)> _pendingLnavInstructions = new();
    private readonly ConcurrentQueue<(int minimum, int maximum, uint climbRate)?> _pendingVnavInstructions = new();
    private readonly ConcurrentQueue<(uint minimum, uint maximum, float acceleration)?> _pendingSpeedInstructions = new();
    ...
    public Aircraft(string callsign, Flightplan fpl, Coordinate startingPosition, float startingCourse, uint startingSpeed, int startingAltitude)
    {
        ...
        Task.Run(async () => await ProcessSpeedInstructionsAsync(_cts.Token));
    }
    ...
    private async Task ProcessVnavInstructionsAsync(CancellationToken token)
    {
        while (!token.IsCancellationRequested)
        {
            ...
            if (instr is null)
                ...
                ExecRestrictAltitude(instr);
        }
    }


    /// <summary>Dequeues speed instructions from the waitlist and executes them.</summary>
    private async Task ProcessSpeedInstructionsAsync(CancellationToken token)
    {
        while (!token.IsCancellationRequested)
        {
            if (!_pendingSpeedInstructions.TryDequeue(out var instr))
            {
                try { await Task.Delay(50, token); }
                catch { return; }
                continue;
            ...
            _triggerSpeedInstruction.WaitOne();
            cancelSpeedWait = false;

            if (instr is null)
                _triggerLnavInstruction.WaitOne();
            else
                ExecRestrictSpeed(instr);
        }
    }
    #endregion
    ...
    private bool cancelSpeedWait = false;
    ...
    private void ExecFlyDirect(object args)
    {
        ...
        _turnRate = _position.DistanceTo(target) > 0.035m ? 360f : turnRate;
        _turnDirection = turnDirection;

        // Wait until within 60ft.
        decimal proximity = GroundSpeed > 30 ? 0.03m : 0.01m;
        Task.Run(async () => await WaitWhileAsync(() => _position.DistanceTo(target) > proximity, () => _turnToCourse = _position.GetBearingDistance(target).bearing));
    }
    ...
    public void RestrictAltitude(int minimum, int maximum, uint climbRate) =>
        _pendingVnavInstructions.Enqueue((Math.Min(minimum, maximum), Math.Max(maximum, minimum), climbRate));

    public void PauseAltitudeUntilWaypoint() =>
        _pendingVnavInstructions.Enqueue(null);
    ...
    public void RestrictSpeed(uint minimum, uint maximum, float acceleration)
    {
        ...
        _pendingSpeedInstructions.Enqueue((Math.Min(minimum, maximum), Math.Max(maximum, minimum), acceleration));
    }

    public void PauseSpeedUntilWaypoint() =>
        _pendingSpeedInstructions.Enqueue(null);
    ...
    public void InterruptSpeed()
    {
        _pendingSpeedInstructions.Clear();
        Thread.Sleep(50);
        ContinueSpeed();
    }
